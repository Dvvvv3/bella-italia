...
app.post('/api/admin/products', requireAdmin, (req, res) => {
  const { id, name, unit, image, stock, price, category_id } = req.body;
  ...
  ).run(id, name, unit || '', image || '', Number(stock) || 0, Number(price) || 0, category_id || null);
  res.json({ ok: true });
});

app.put('/api/admin/products/:id', requireAdmin, (req, res) => {
  const { name, unit, image, stock, price, category_id } = req.body;
  ...
  db.prepare(
    `UPDATE products SET name=?, unit=?, image=?, stock=?, price=?, category_id=? WHERE id=?`
  ).run(name, unit, image, Number(stock) || 0, Number(price) || 0, category_id || null, req.params.id);
  res.json({ ok: true });
});
